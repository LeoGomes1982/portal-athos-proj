...
-- Create a function to check if role change is allowed
CREATE OR REPLACE FUNCTION public.is_role_change_allowed(old_role text, new_role text)
RETURNS boolean
LANGUAGE sql
...
AS $$
  SELECT 
    CASE 
      WHEN old_role = new_role THEN true
